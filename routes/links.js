...
router.post('/', [ 
    ...
    if (req.body.userId) {
        ...
        .catch(err => res.status(404).json({msg: "User not found", err}));
...
], validateLink, compareLink, (req, res) => {
    Link.findByIdAndUpdate(req.params.id, {short: req.body.short}, {new: true, runValidators: true, useFindAndModify: false})
       .then( link => res.json(link))
